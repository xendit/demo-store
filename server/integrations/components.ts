...
/**
 * Creates a Session for Components integration using the provided data.
 */
const makeSessionForComponents = async (
  ...
  const payload = {
    ...
    // Set the right mode and origins for Components integration
  ...
  const session = (await response.json()) as ComponentsSession;
  return session;
};

export default makeSessionForComponents;
